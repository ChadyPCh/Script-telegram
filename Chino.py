...
api_id = 2339304          # aqui va el id de tu api
api_hash = 'e81d65411db5fcc14a7f0c83779db65d'		 # aqui el hash de tu api
# fill in your own details here

phone = '+5355346706'  # Cambiarlo
session_file = 'Chino'  # use your username if unsure
MOBS = False
HP = 0
LVL = 0
a = 0
...
client = TelegramClient('ch1n0_98', api_id, api_hash,sequential_updates=True)
...
async def new_group_handle(event):
    ...
    if "Chito" in event.raw_text:
        Forest = Swamp = Valley = RandomQuest = False
        a=0
    
    if "Chiq" in event.raw_text:
        RandomQuest = True
        Forest = Swamp = Valley = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Chifor" in event.raw_text:
        Forest = True
        Swamp = Valley = RandomQuest = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Chis" in event.raw_text:
        Swamp = True
        Forest = Valley = RandomQuest = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Chiv" in event.raw_text:
        Forest = Swamp = RandomQuest = False
        Valley = True
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "ChiOn" in event.raw_text:
        MOBS = True

    if "ChiOf" in event.raw_text:
        MOBS = False

    if "Chiff" in event.raw_text:
        a=2
        await client.send_message('chtwrsbot','▶️Fast fight')
...
print(time.asctime(), '-', 'Stared Chino...')
client.start(phone)
