...
client = TelegramClient('elpinwy', api_id, api_hash,sequential_updates=True)
...
async def new_quest_handle(event):
 ...
 #arenas
    ...
    if "Not enough stamina. " in event.raw_text:
        a=0
        time.sleep(3)
        await client.send_message('chtwrsbot','/g_def TNT')
...
async def new_group_handle(event):
    ...
    if "Kto" in event.raw_text:
        Forest = Swamp = Valley = RandomQuest = False
        a=0
    
    if "Kq" in event.raw_text:
        RandomQuest = True
        Forest = Swamp = Valley = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Kfor" in event.raw_text:
        Forest = True
        Swamp = Valley = RandomQuest = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Ks" in event.raw_text:
        Swamp = True
        Forest = Valley = RandomQuest = False
        await client.send_message('chtwrsbot','🗺Quests')
        a=1

    if "Kv" in event.raw_text:
    ...
    if "KOn" in event.raw_text:
        MOBS = True

    if "KOf" in event.raw_text:
        MOBS = False

    if "Kff" in event.raw_text:
        a=2
        await client.send_message('chtwrsbot','▶️Fast fight')
...
print(time.asctime(), '-', 'Stared Kawasky...')
client.start(phone)
